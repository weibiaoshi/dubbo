...
public class Register extends Shell {
	...
	protected String doExecute(Map<String, Object> context) throws Exception {
		...
		for (Map.Entry<String, String[]> entry : params.entrySet()) {
			if (entry.getKey() != null && entry.getKey().length() > 0 && entry.getValue() != null && entry.getValue().length > 0 && entry.getValue()[0] != null && entry.getValue()[0].length() > 0) {
				...
				for (Map.Entry<String, String> e : url2query.entrySet()) {
					...
					if (StringUtils.isBlank(app)) {
						throw new IllegalStateException("No application for service(" + serviceName + "): " + e.getKey() + "?" + e.getValue());
					}
				}
